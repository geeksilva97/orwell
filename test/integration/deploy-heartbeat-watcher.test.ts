...
import { makeWatchFromWatchIds } from '../../lib/commands/heartbeat.ts';
...
describe('Heartbeat watcher integration', () => {
  ...
  it('deploys a heartbeat watcher and verifies its structure', async (t: TestContext) => {
    ...
    const { data, status } = await esClient.get(`/_watcher/watch/${HEARTBEAT_ID}`) as {
    ...
    const inputs = (storedWatch.input as { chain: { inputs: unknown[] } }).chain.inputs;
    ...
    for (let i = 0; i < WATCH_IDS.length; i++) {
      ...
      const searchInput = (inputs[i + 1] as Record<string, { search: { request: { indices: string[]; body: Record<string, unknown> } } }>)[watchId];
    ...
    const transformScript = (storedWatch.transform as { chain: Array<{ script: { lang: string; source: string } }> }).chain[0]!.script;
    ...
    const actions = storedWatch.actions as { send_slack_message: Record<string, unknown> };
    const slackAction = actions.send_slack_message;
  ...
  it('executes the heartbeat and shouldSendMessage is false when no history exists', async (t: TestContext) => {
    ...
    ) as { data: { watch_record: { result: { condition: { met: boolean }; actions: Array<{ id: string; status: string }> } } } };
