...
function action(options) {
  ...
  // xor operation to determin when to perform both remove and push
  ...
  deployAlertsService.execute({
    ...
    if (alertsToDelete.length > 0) {
      console.log(JSON.stringify(alertsToDelete, null, 2));
      ...
      // return deleteAlertsService.execute({
      //   envTarget,
      //   serverTarget
      // })
      //   .then(() => {
      //     container.logger.info(`${alertsToDelete.length} alerts deleted successfully!`);
      //   })
      //   .catch(error => {
