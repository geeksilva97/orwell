...
function action(options) {
  ...
  Promise.allSettled([
    ...
    const [deployAlertsResult, deleteAlertsResult] = results;

    if (deployAlertsResult.status === 'rejected') {
      container.logger.error('Could not deploy alerts');
      container.logger.error(deployAlertsResult.reason);
    }

    if (deleteAlertsResult.status === 'rejected') {
      container.logger.error('Could not delete alerts');
      container.logger.error(deleteAlertsResult.reason);
    }

    if (shouldPush && deployAlertsResult.status === 'fulfilled') {
      if (deployAlertsResult.value.length === 0) {
        container.logger.info('No alerts deployed!');
        return
      }

      container.logger.info(`${deployAlertsResult.value.length} alerts deployed successfully!`);
    }

    if (shouldDelete && deleteAlertsResult.status === 'fulfilled') {
      if (deleteAlertsResult.value.length === 0) {
        container.logger.info('No alerts deleted!');
        return
      }

      container.logger.info(`${deleteAlertsResult.value.length} alerts deleted successfully!`);
    }
  });
}
