...
function heartbeatUpAction(parseConfigFn, options) {
  ...
  const orwellConfigBasePath = options.configPath || path.join(process.cwd(), 'orwell.js');

  const { baseDir, heartbeat: heartBeatConfig } = parseConfigFn(orwellConfigBasePath);
